      ...
      <div style={cardContainerStyle}>
        {allSongs.map((song, i) => (
          <React.Fragment key={i}>
            <a className='songCards' style={{ cursor: 'pointer' }} id={song.song_id} onClick={playSong} >
              <div style={{ ...cardStyles, pointerEvents: 'none' }}>
                <div style={{ justifySelf: 'start', pointerEvents: 'none' }}>{song.title}
                  <div style={{ marginTop: '5px', pointerEvents: 'none', justifySelf: 'center', fontSize: '12px', color: '#b3b3b3' }}>{song.artist}
                    <span style={{ marginLeft: '10px', pointerEvents: 'none' }}>•</span>
                    <span style={{ marginLeft: '10px', justifySelf: 'center', pointerEvents: 'none', color: '#b3b3b3', display: 'inline' }}>{song.album}</span>
                  </div>
                </div>
