...
#     playlists = db.session.query(Song.title,Album.title,Artist.name,Song.song_url,Song.song_length,Album.image_url,Artist.image_url,Artist.bio,Playlist.name).select_from(Playlist).join(PlaylistSong).join(Song).join(Album).join(Artist).filter(Playlist.user_id==userid).all()
    
#     payload = [{"song_title":playlist[0], "album_title":playlist[1],"artist_name":playlist[2],"song_url":playlist[3],"song_length":playlist[4],"album_image":playlist[5],"artist_image":playlist[6],"artist_bio":playlist[7],"playlist_name":playlist[8]}for playlist in playlists]
